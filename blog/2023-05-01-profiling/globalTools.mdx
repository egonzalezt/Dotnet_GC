...
Before runnig our [Sample Code](https://gist.github.com/egonzalezt/ce6f6a5540fe1186efd0a9a80f4d02f3) you need to install the following tools:
...
The required tools mentioned before needs the PID of your .NET application to get the PID run:
...
The results have sense if knowing that out code creates a list of strings and store a very long strong.
...
are the responsibles of the memory leak, which means that we need to found a list of strings that are producing the leak in our [Code](https://gist.github.com/egonzalezt/ce6f6a5540fe1186efd0a9a80f4d02f3)
...
This command creates a file with extension `.gcdump` that can be open with VisualStudio

![gcdumpvs](./gcdumpvs.png)

In this case our biggest object is `List<String>` of size `193,442,434 byes` if you click on that object VisualStudio gives the PathToRoot and the results are exactly the same as the firt method using dotnet-dump, a list of strings are the responsibles:
...
:::tip
The main reason why this code never releases memory is because when GC starts the process of Collecting Garbage, the `List<String>` still being use by the process and cannot be deleted, for that reason the list pass to the different generations Gen 0 > Gen 1 > Gen 2 or LOH, is important to keep in mind when the objects are going to be destroyed to avoid this problems.
:::
