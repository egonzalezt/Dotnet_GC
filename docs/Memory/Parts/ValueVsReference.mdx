...
When the CalculateTotalYears method is removed from the stack, `DateTime` is no longer accessible and will remain until the GarbageCollector clears the stack.
...
Using global variables are variables that need to be accessible from different parts of your code, and for that reason need to be created on the heap to still be alive during the class execution or program. Finally, objects that have different ValueTypes like `int`, `float`, `decimal` these variables and the object itself live on the heap.
...
maxAge and date are needed in the Anonymous function and the Main method so is needed to put these variables on the heap. Finally, when the Anonymous function ends all the objects on the heap that don't have a reference are destroyed by the GarbageCollector.
